...
subroutine firststep()  !QUESTION: How to go about Euler Method
end subroutine firststep	
	                
			
subroutine secondstep() !QUESTION: How to go about AB 
...
subroutine momentum(u, v, h, us0, vs0, f, g, dT, d, Nx, Ny)
    implicit none

    !Declare variables
    real, intent(inout) :: u(Nx, Ny), v(Nx, Ny), h(Nx, Ny)
    real, intent(in) :: us0(Nx, Ny), vs0(Nx, Ny), f, g, dT, d
    integer, intent(in) :: Nx, Ny
    integer :: i, j
    real :: alp0, bet0, gam0, del0, eps0, ken0, ght0, phi0 !QUESTION: Definitions?

    ! Update u-momentum equation
    
    do j = 2, Ny - 1
        do i = 2, Nx - 1
            u(i, j) = u(i, j) + dT *((alp0 * vs0(i, j) + bet0 * vs0(i - 1, j + 1) + &
     &            gam0 * vs0(i - 1, j) + del0 * vs0(i + 1, j)) - (eps0 * (us0(i + 1, j + 1) - us0(i - 1, j + 1)) - &
     &            (ken0 + ght0))) / d
        end do
    end do

    ! Update v-momentum equation

    do j = 2, Ny - 1
        do i = 2, Nx - 1
            v(i, j) = v(i, j) - dT * ((gam0 * us0(i + 1, j + 1) + del0 * us0(i, j + 1) + &
            alp0 * us0(i, j - 1) + bet0 * us0(i + 1, j - 1)) - (phi0 * (vs0(i + 1, j + 1) - &
            vs0(i + 1, j - 1)) - (ken0 + ght0))) / d
        end do
    end do
...
subroutine height()	! Can we please work on this? 
